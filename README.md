...
```
Validates each unvalidated `PotentialInstrument` via Yahoo Finance scrape. Successfully scraped instruments are created as `Instrument` records with price history (daily, weekly, and monthly) and dividends, then marked validated. Runs 3 parallel Chromium workers by default. A circuit breaker halts all workers automatically if 5 consecutive failures occur (e.g. rate limiting).
